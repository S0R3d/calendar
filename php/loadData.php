    ...
    foreach ($rtrn as $i => $event) {
        ...
        if (!$real_id) { // REAL EVENT
            if ($sD == $fD and ($sT == $fT and $fT == "00:00")) { // Fgg
                ...
                $RESULT .= '<div class="event NFgg" event-id="'.$event["id"].'" style="order: '.($i+1).';"><div class="icon"></div><div class="hours">'.$sT.'</div><div class="title">'.$event["title"].'</div></div>';
            } else if ($sD != $fD and $sT != $fT) { // NFggs 
                $RESULT .= '<div class="event NFggs" event-id="'.$event["id"].'" style="order: '.($i+1).';"><div class="hours">'.$sT.'</div><div class="title">'.$event["title"].'</div></div>';
            }
        } else { // END EVENT
